...
class ReportModel {
    ...
    static async create(data) {
        const { name, description, sql_query, chart_type, chart_config, visual_config, created_by, is_public } = data;
        const [result] = await pool.execute(
            'INSERT INTO reports (name, description, sql_query, chart_type, chart_config, visual_config, created_by, is_public) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
            [name, description || '', sql_query, chart_type || 'table', JSON.stringify(chart_config || {}), JSON.stringify(visual_config || null), created_by, is_public ? 1 : 0]
        );
        return result.insertId;
    }

    static async update(id, data) {
        const { name, description, sql_query, chart_type, chart_config, visual_config, is_public } = data;
        await pool.execute(
            'UPDATE reports SET name=?, description=?, sql_query=?, chart_type=?, chart_config=?, visual_config=?, is_public=? WHERE id=?',
            [name, description || '', sql_query, chart_type, JSON.stringify(chart_config || {}), JSON.stringify(visual_config || null), is_public ? 1 : 0, id]
        );
    }
